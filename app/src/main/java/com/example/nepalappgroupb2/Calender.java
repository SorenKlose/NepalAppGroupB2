...
public class Calender extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstaceState) {
        ...
        for(int i = 0; i < test.length; i++) {
            cardArray.add(new CardElement(test[i]));
        }
    ...
    RecyclerView.Adapter adapter = new RecyclerView.Adapter() {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            ...
            int resID = cardElement.getBgImgIDFromTitle(cardArray.get(position).getCardTitle(), getContext());
            if(resID != 0) {
                bgImage.setImageResource(resID);
            } else {
                //if the image does not exist in "drawable" we assign a default image
                bgImage.setImageResource(cardElement.getBgImgIDFromTitle(cardElement.getDefaultImg(), getContext()));
            }
        }
